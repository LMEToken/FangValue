...
- (void)addTableview
{
    ...
    self.DetailTableview = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, 320, 500) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==3) {
        ...
        UITextView *fpurposte = [[UITextView alloc]initWithFrame:CGRectMake(85, 3, self.view.bounds.size.width-85, 77)];
        fpurposte.text =  [datadic objectForKey:@"statge"];
        fpurposte.editable =NO;
    ...
    if (indexPath.row ==8) {
        ...
        for (int i =0; i<[[datadic objectForKey:@"docmentlist"] count]; i++) {
            ...
            [presonname[i] setFont:[UIFont fontWithName:KUIFont size:10]];
...
{
    //    FvaProCrowdDataVC *crowdView = [[FvaProCrowdDataVC alloc]init];
    ChatWithFriendViewController *vc =[[ChatWithFriendViewController alloc]init];
    NSDictionary *dic=[[datadic objectForKey:@"dg"] objectAtIndex:crowdbt.tag];
    ...
    vc.isPushString=@"is";
    NSArray *arr = [NSArray arrayWithObjects:[dic objectForKey:@"dgid"],@"1", nil];
