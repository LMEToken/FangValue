...
- (void)messageStyleButtonClicked:(UIButton *)sender {
    switch (sender.tag) {
        ...
        case 2:
        {
            ...
            if (sender.selected) {
                NSLog(@"分享被点击");
                  [[NSNotificationCenter defaultCenter] postNotificationName:@"sendimage" object:nil];
               [self.messageInputTextView resignFirstResponder];
            }else{
                NSLog(@"分享没被点击");
                  [[NSNotificationCenter defaultCenter] postNotificationName:@"sendimage" object:nil];
...
- (void)setupMessageInputViewBarWithStyle:(ZBMessageInputViewStyle )style{
    ...
    // 发送语音
    self.voiceChangeButton = [self createButtonWithImage:[UIImage imageNamed:@"ToolViewKeyboard"]
                                                 HLImage:nil];
   
    [self.voiceChangeButton setImage:[UIImage imageNamed:@"ToolViewInputVoice"]
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        [self setup];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(removborad) name:@"removborad"object:nil];
    }
    return self;
}
-(void)removborad
...
//    }
    ...
    if ([self.delegate respondsToSelector:@selector(didSendFaceAction2)]) {
        [self.delegate didSendFaceAction2];
