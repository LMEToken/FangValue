...
- (void)setState:(PRState)state animated:(BOOL)animated{
    ...
    if (_state != state) {
        ...
        if (_state == kPRStateLoading) {    //Loading
            ...
            //2014.05.05 chenlihua
                  _stateLabel.text = NSLocalizedString(@"收取中", @"");
            } else {
...
//
//    float a = y > self.contentSize.height ? self.contentSize.height : y;
//    y=a;
    ...
    //2014.05.27 chenlihua 修改从方创人界面，点击项目方，投资方，对接群部分时，TableView从左侧进入的动画
    /*
...
- (void)tableViewDidEndDragging:(UIScrollView *)scrollView {
    ...
    if (_headerView.state == kPRStatePulling) {
       ...
       [UIView animateWithDuration:kPRAnimationDuration animations:^{
           //2014.08.15 chenlihua 将方创界面刷新时，总是有由左到右的动画出现。
...
- (void)flashMessage:(NSString *)msg{
    ...
    if (_msgLabel == nil) {
        ...
        _msgLabel.font = [UIFont fontWithName:@"Arial-BoldMT" size:14];
