...
- (id)initWithFrame:(CGRect)frame
{
   self = [super initWithFrame:frame];
    if (self) {
        ...
        faceView.contentSize = CGSizeMake((85/28)*320, 190);
        ...
        for (int i = 1; i<85; i++) {
            ...
            //计算每一个表情按钮的坐标和在哪一屏
            faceButton.frame = CGRectMake((((i-1)%28)%7)*44+6+((i-1)/28*320), (((i-1)%28)/7)*44+8, 30, 30);
            
    [faceButton setImage:[UIImage imageNamed:[NSString stringWithFormat:@"f%03d",i]]
    ...
    [faceView addSubview:faceButton];
        ...
        facePageControl.numberOfPages = 85/28;
...
- (void)faceButton:(id)sender {
    ...
    if (self.inputTextView) {
        ...
        [self.inputTextView setContentOffset:CGPointMake(0, self.inputTextView.contentSize.height - 22) animated:YES];
