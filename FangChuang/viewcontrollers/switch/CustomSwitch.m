...
-(void)setStatus:(CustomSwitchStatus)status
{
    ...
    if ([_delegate respondsToSelector:@selector(customSwitchSetStatus:custom:)]) {
        [_delegate customSwitchSetStatus:_status custom:self];
    }
...
-(id)initWithOnImage:(UIImage*)onImage offImage:(UIImage*)offImage arrange:(CustomSwitchArrange)arrange withFrame:(CGRect)frame
{
    self = [super initWithFrame:frame];
    if (self == nil) {
        return self;
    ...
    _customSwitch = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(frame), CGRectGetHeight(frame))];
...
//    [_offButton setBackgroundColor:[UIColor cl]];
    _currentTranslationX = 0;
    
    
    if (_arrange == CustomSwitchArrangeONLeftOFFRight) {
        
        _onButton.frame = CGRectMake(0, 0, _onImage.size.width / 2., _onImage.size.height / 2.);
        _offButton.frame = CGRectMake(_onImage.size.width / 2. - _onImage.size.width / 2.,0, _offImage.size.width / 2., _offImage.size.height / 2.);
    }else{
        
        _offButton.frame = CGRectMake(0, 0, _offImage.size.width / 2., _offImage.size.height / 2.);
        _onButton.frame = CGRectMake(_onImage.size.width / 2. - _onImage.size.width / 2., 0, _onImage.size.width / 2. , _onImage.size.height / 2.);
...
- (void)drawRect:(CGRect)rect
{
    if (!first) {
        _customSwitch.backgroundColor = [UIColor clearColor];
        [_customSwitch setFrame:CGRectMake(0, 0, CGRectGetWidth(rect), CGRectGetHeight(rect))];
        [_onButton setImage:_onImage forState:UIControlStateNormal];
        [_offButton setImage:_offImage forState:UIControlStateNormal];
        [_onButton addTarget:self action:@selector(switchBtnClicked) forControlEvents:UIControlEventTouchUpInside];
        [_offButton addTarget:self action:@selector(switchBtnClicked) forControlEvents:UIControlEventTouchUpInside];
        
        _leftRect = CGRectMake(-(_onImage.size.width / 2.- _onImage.size.height / 2.), 0, _onImage.size.width/ 2., _onImage.size.height/ 2.);
        _middleRect = CGRectMake(0, 0, _onImage.size.width/ 2., _onImage.size.height/ 2.);
        ...
        _minusTranslate = _onImage.size.width / 2. - _onImage.size.height / 2.;
        _currentTranslationX = 0;
        if (_arrange == CustomSwitchArrangeONLeftOFFRight) {
