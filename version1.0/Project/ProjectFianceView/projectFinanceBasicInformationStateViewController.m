//
//  projectFinanceBasicInformationStateViewController.m
//  FangChuang
//
...
@interface projectFinanceBasicInformationStateViewController ()

@end

@implementation projectFinanceBasicInformationStateViewController
...
-(void)initChooseButton{
    turnButtonArray=[[NSMutableArray alloc]initWithObjects:@"上会10%",@"立项20%",@"TS35%",@"DD55%",@"SPA70%",@"股权交割90%",@"到账 100%", nil];
    ...
    //从添加页面进来的
    if ([jumpFlagString isEqualToString:CLH_NSSTRING_FINANCE_RIGHT_ADD_STATE_FLAG]) {
        ...
        if (jumpStr.length==0) {
            ...
            chooseIndex=[[NSMutableArray alloc] initWithArray:[[NSUserDefaults standardUserDefaults] objectForKey:CLH_DEFAULT_FINANCE_STATE_INDEX]];
        ...
        //跟踪页面
    }else if ([jumpFlagString isEqualToString:CLH_NSSTRING_FINANCE_DETAIL_ADD_STATE_FLAG]){
        if (!chooseIndex) {
            chooseIndex=[[NSMutableArray alloc]init];
        }
...
-(void)doClickRightButton:(UIButton *)btn
{
    ...
    if ([jumpFlagString isEqualToString:CLH_NSSTRING_FINANCE_RIGHT_ADD_STATE_FLAG]) {
        ...
        [chooseIndexDefault setObject:chooseIndex forKey:CLH_DEFAULT_FINANCE_STATE_INDEX];
        ...
        [currency setObject:chooseStr forKey:CLH_DEFAULT_FINANCE_STATE];
    ...
    }else if ([jumpFlagString isEqualToString:CLH_NSSTRING_FINANCE_DETAIL_ADD_STATE_FLAG])
    {
        ...
        NSUserDefaults *currency=[NSUserDefaults standardUserDefaults];
        [currency setObject:chooseStr forKey:CLH_DEFAULT_FINANCE_DETAIL_STATE];
        [currency synchronize];
