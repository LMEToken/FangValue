//
//  HomeInvestorMoneyViewController.m
//  FangChuang
//
...
@interface HomeInvestorMoneyViewController ()

@end

@implementation HomeInvestorMoneyViewController
...
-(void)initExplainTitle
{
    ...
    UILabel *explainLabel=[[UILabel alloc] initWithFrame:CGRectMake(15, 1, 80, 30)];
    explainLabel.text=@"选择融资金额";
...
{
    [[NetManager sharedManager] getProFilterOptWithusername:[[NSUserDefaults standardUserDefaults]objectForKey:@"tele"] apptoken:[[UserInfo sharedManager] apptoken] rflag:@"5" hudDic:nil success:^(id responseDic) {
...
-(void)doClickRightButton:(UIButton *)btn
{
    ...
    [currency setObject:chooseStr forKey:CLH_DEFAULT_INVEST_MONEY];
    ...
    [server setObject:serverStr forKey:@"server-money-i"];
    [server synchronize];

    
    HomeInvestorViewController *investorView=[[HomeInvestorViewController alloc]init];
    [self.navigationController pushViewController:investorView animated:NO];
