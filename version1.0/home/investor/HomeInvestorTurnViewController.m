//
//  HomeInvestorTurnViewController.m
//  FangChuang
//
...
@interface HomeInvestorTurnViewController ()

@end

@implementation HomeInvestorTurnViewController
...
{
    [[NetManager sharedManager] getProFilterOptWithusername:[[NSUserDefaults standardUserDefaults]objectForKey:@"tele"] apptoken:[[UserInfo sharedManager] apptoken] rflag:@"1" hudDic:nil success:^(id responseDic) {
        
        NSLog(@"-turn--responseDic--%@",responseDic);
        NSMutableArray *dataArray=[responseDic objectForKey:@"data"];
        NSLog(@"--turn--dataArray.count-%i",dataArray.count);
...
-(void)doClickRightButton:(UIButton *)btn
{
    ...
    [currency setObject:chooseStr forKey:CLH_DEFAULT_INVEST_TURN];
    ...
    [server setObject:serverStr forKey:@"server-turn-i"];
    [server synchronize];
