...
@interface RaisefundsVC ()

@end

@implementation RaisefundsVC
...
- (void)viewDidLoad
{
    ...
    if ([[[UserInfo sharedManager] usertype]isEqualToString:@"1"]) {
        ...
        heradllable.text = @"投资人关注领域";
    }else
    {
           savearr = [NSArray arrayWithObjects:@"未启动",@"天使轮",@"Pre-A轮",@"A轮",@"B轮",@"C轮以后",@"并购",nil];
        
            heradllable.text = @"创业者融资状态";
    }
    ...
    for (int i = 0; i<count/line; i++) {
        for (int j =0;j<line; j++) {
            ...
            typebutton2[i][j]= [[UIButton alloc]initWithFrame:CGRectMake(20+75*j, 60+70*i, 70, 50)];
            ...
            typebutton[i][j]= [[UILabel alloc]initWithFrame:CGRectMake(20+75*j, 60+70*i, 70, 50)];
...
-(void)goback
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"changeRaise" object:typearr];
