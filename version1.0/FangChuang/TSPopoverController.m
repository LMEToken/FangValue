...
{    
    ...
    CGPoint applicationFramePoint = CGPointMake(screenRect.origin.x,0-screenRect.origin.y-20);
...
- (CGRect) contentFrameRect:(CGRect)contentFrame senderPoint:(CGPoint)senderPoint
{
    ...
    float screenWidth1 = screenRect.size.width;
    float screenHeight1 = screenRect.size.height - screenRect.origin.y;
    ...
    if(self.arrowPosition == TSPopoverArrowPositionVertical){
        ...
        if(arrowDirection == TSPopoverArrowDirectionTop){
            ...
            if((popoverY+popoverHeight) > screenHeight1){
                contentFrameRect.size.height = screenHeight1 - (screenRect.origin.y + popoverY + titleLabelheight + (OUTER_MARGIN*2+MARGIN*2));
            }
        }
        ...
        if(contentFrameRect.size.height > screenHeight1 - (OUTER_MARGIN*2+MARGIN*2)){
            contentFrameRect.size.height = screenHeight1- (OUTER_MARGIN*2+MARGIN*2);
        }
        ...
        if(arrowDirection == TSPopoverArrowDirectionLeft){
            ...
            if((popoverX+popoverWidth)> screenWidth1 - (OUTER_MARGIN*2+MARGIN*2)){
                contentFrameRect.size.width = screenWidth1 - popoverX - ARROW_SIZE - (OUTER_MARGIN*2+MARGIN*2);
            }
        }
