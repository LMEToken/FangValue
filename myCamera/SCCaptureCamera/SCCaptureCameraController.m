...
- (void)viewDidLoad
{
    ...
    if (CGRectEqualToRect(_previewRect, CGRectZero)) {
      ...
      //  self.previewRect = CGRectMake(0, 0, SC_APP_SIZE.width, SC_APP_SIZE.width + CAMERA_TOPVIEW_HEIGHT);
        if (isHigherThaniPhone4_SC){
            self.previewRect = CGRectMake(0, 0, SC_APP_SIZE.width, SC_APP_SIZE.width + CAMERA_TOPVIEW_HEIGHT+100);
        }else{
            self.previewRect = CGRectMake(0, 0, SC_APP_SIZE.width, SC_APP_SIZE.width + CAMERA_TOPVIEW_HEIGHT);
        }
    }
    ...
    if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        if (isHigherThaniPhone4_SC) {
            UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, CAMERA_TOPVIEW_HEIGHT, self.view.frame.size.width, self.view.frame.size.width+100)];
            imgView.clipsToBounds = YES;
            imgView.contentMode = UIViewContentModeScaleAspectFill;
            imgView.image = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"nil" ofType:@"jpg"]];
...
- (void)addCameraMenuView {
 ...
 //   CGFloat cameraBtnLength = 90;
    //2014.09.15 chenlihua 拍照按钮变成小一点
    
    CGFloat cameraBtnLength = (isHigherThaniPhone4_SC ? 40 : 90);
