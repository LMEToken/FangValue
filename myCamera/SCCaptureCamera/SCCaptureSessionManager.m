...
//        [SCCommon saveImageToPhotoAlbum:image];
       ...
       // CGSize size = CGSizeMake(squareLength * 2, squareLength * 2);
        ...
        //2014.09.12 chenlihua 将图片预览的时候上下栏的黑框去掉。
        
        CGSize size = CGSizeMake(squareLength * 2, squareLength * 3);
...
- (AVCaptureVideoOrientation)avOrientationForDeviceOrientation:(UIDeviceOrientation)deviceOrientationCamera
{
	AVCaptureVideoOrientation result = (AVCaptureVideoOrientation)deviceOrientationCamera;
	if ( deviceOrientationCamera == UIDeviceOrientationLandscapeLeft )
		result = AVCaptureVideoOrientationLandscapeRight;
	else if ( deviceOrientationCamera == UIDeviceOrientationLandscapeRight )
...
- (void)switchGrid:(BOOL)toShow {
    ...
    if (isHigherThaniPhone4_SC) {
        ...
        for (int i = 2; i < 4; i++) {
            CGRect frame = CGRectZero;
            frame = CGRectMake((i + 1 - 2) * eachAreaLength, headHeight, 1, squareLength+100);
            [SCCommon drawALineWithFrame:frame andColor:[UIColor whiteColor] inLayer:_preview.layer];
        }
